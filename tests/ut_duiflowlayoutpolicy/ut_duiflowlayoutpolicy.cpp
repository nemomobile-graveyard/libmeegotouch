...
#include "ut_duiflowlayoutpolicy.h"
...
Ut_DuiFlowLayoutPolicy::Ut_DuiFlowLayoutPolicy()
    : m_mockLayout(0)
    , m_policy(0)
    , m_mockItem100(0)
    , m_mockItem200(0)
...
void Ut_DuiFlowLayoutPolicy::initTestCase()
{
    static int argc = 1;
    static char *argv[1] = { (char *) "./ut_duiflowlayoutpolicy" };
    app = new DuiApplication(argc, argv);
    app->setLayoutDirection(Qt::LeftToRight);
    appWin = new DuiApplicationWindow;
}

void Ut_DuiFlowLayoutPolicy::cleanupTestCase()
{
    delete appWin;
    delete app;
}

void Ut_DuiFlowLayoutPolicy::init()
{
    Q_ASSERT(0 == m_mockLayout);
    m_mockLayout = new DuiLayout();
    m_mockLayout->setContentsMargins(0, 0, 0, 0);
    m_form = new QGraphicsWidget;
    m_form->setLayout(m_mockLayout);
    Q_ASSERT(0 == m_policy);
    m_policy = new DuiFlowLayoutPolicy(m_mockLayout);
...
void Ut_DuiFlowLayoutPolicy::cleanup()
{
    Q_ASSERT(0 != m_mockItem100);
...
void Ut_DuiFlowLayoutPolicy::testOneItemLayout()
{
...
/** Test with two items, layout is wide enough to keep both items in a single
    row. */
void Ut_DuiFlowLayoutPolicy::testTwoItemsHorizontally()
{
...
/** Test with two items, layout is so narrow that each item should end up in
    its own row. */
void Ut_DuiFlowLayoutPolicy::testTwoItemsVertically()
{
    m_policy->addItem(m_mockItem100);
    m_policy->addItem(m_mockItem200);
    m_form->setGeometry(QRectF(0, 0, m_mockItem100->preferredWidth() +
...
/** Test with three items, layout is so narrow that the items should end up in
    two rows. */
void Ut_DuiFlowLayoutPolicy::testHorizontallyAndVertically()
{
    m_policy->addItem(m_mockItem100);
    m_policy->addItem(m_mockItem200);
...
void Ut_DuiFlowLayoutPolicy::testReorderingFlow()
{
...
/** Test with margins and three items, layout is so narrow that the items should end up in
    two rows. */
void Ut_DuiFlowLayoutPolicy::testMargins()
{
    m_policy->addItem(m_mockItem100);
    m_policy->addItem(m_mockItem200);
    m_policy->addItem(m_mockItem300);
...
/** Test with margins and two items, layout is so narrow that the items should end up in
    two rows. */
void Ut_DuiFlowLayoutPolicy::testMarginsAndTwoItems()
{
    m_policy->addItem(m_mockItem100);
    m_policy->addItem(m_mockItem200);
...
void Ut_DuiFlowLayoutPolicy::testSpacing()
{
    m_policy->addItem(m_mockItem100);
    m_policy->addItem(m_mockItem200);
    m_policy->addItem(m_mockItem300);
...
void Ut_DuiFlowLayoutPolicy::testHorizontalVerticalSpacing()
{
    m_policy->addItem(m_mockItem100);
    m_policy->addItem(m_mockItem200);
    m_policy->addItem(m_mockItem300);
...
void Ut_DuiFlowLayoutPolicy::testHorizontalJustification_data()
{
...
void Ut_DuiFlowLayoutPolicy::testEmptyCount()
{
    QCOMPARE(m_policy->rowCount(), 0);
}
void Ut_DuiFlowLayoutPolicy::testHorizontalJustification()
{
...
void Ut_DuiFlowLayoutPolicy::testTwoByTwoThenRemoveThird_data()
{
...
void Ut_DuiFlowLayoutPolicy::testTwoByTwoThenRemoveThird()
{
...
void Ut_DuiFlowLayoutPolicy::testBigItemInSmallLayout()
{
...
void Ut_DuiFlowLayoutPolicy::testMarginsAndMinimumHeight()
{
...
void Ut_DuiFlowLayoutPolicy::testAddingRemovingAdding()
{
    new DuiBasicLayoutAnimation(m_mockLayout);
