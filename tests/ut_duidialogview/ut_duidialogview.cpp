...
#include "ut_duidialogview.h"

#include <QGraphicsLayout>

void Ut_DuiDialogView::init()
{
}

void Ut_DuiDialogView::cleanup()
{
}

void Ut_DuiDialogView::initTestCase()
{
    static int argc = 1;
    static char *app_name[1] = { (char *) "./ut_duidialogview" };
    app = new DuiApplication(argc, app_name);

    controller = new DuiDialog();
    subject = new DuiDialogView(controller);
    controller->setView(subject);
    model = controller->model();
}

void Ut_DuiDialogView::cleanupTestCase()
{
    delete controller;
    controller = 0;
...
void Ut_DuiDialogView::closeButtonVisibility()
{
    QGraphicsWidget *dialogCloseButton;

    dialogCloseButton = fetchWidget(*controller, "DuiDialogCloseButton");
...
void Ut_DuiDialogView::titleBarVisibility()
{
    QGraphicsWidget *dialogTitleBar = 0;
    QGraphicsWidget *widget = 0;

    dialogTitleBar = fetchWidget(*controller, "DuiDialogTitleBar");
    ...
    widget = fetchWidget(*controller, "DuiDialogTitleBar");
    QVERIFY(widget == 0);
...
QGraphicsWidget *Ut_DuiDialogView::fetchWidget(QGraphicsWidget &widget,
...
QGraphicsWidget *Ut_DuiDialogView::fetchWidget(QGraphicsLayout &layout,
        const QString &objectName) const
{
    QGraphicsWidget *targetWidget = 0;
...
QTEST_APPLESS_MAIN(Ut_DuiDialogView)
