...
#include "ut_duiappletinstancemanager.h"
...
{
    Ut_DuiAppletInstanceManager::callServiceName = service;
    Ut_DuiAppletInstanceManager::callObjectPath = path;
    Ut_DuiAppletInstanceManager::callInterface = interface;
}
...
// QDBusPendingCall stubs (used by DuiAppletInstanceManager)
...
{
    if (Ut_DuiAppletInstanceManager::captureCalls) {
        QList<QVariant> args;
        if (arg1.isValid()) {
            args.append(arg1);
            if (arg2.isValid()) {
        ...
        Ut_DuiAppletInstanceManager::callMethods.append(method);
        Ut_DuiAppletInstanceManager::callArguments.append(args);
    }
...
{
    return Ut_DuiAppletInstanceManager::callError;
}

// QDBusPendingReply stubs (used by DuiAppletInstanceManager)
...
{
    return Ut_DuiAppletInstanceManager::replyDataString;
}

// DuiAppletInstanceManagerDBusAdpator stubs
DuiAppletInstanceManagerDBusAdaptor::DuiAppletInstanceManagerDBusAdaptor(DuiAppletInstanceManager *parent) : QDBusAbstractAdaptor(parent)
{
}
...
void DuiAppletInstanceManagerDBusAdaptor::instantiateAppletFromPackage(const QString &, const QMap<QString, QVariant> &)
{
}

DuiAppletHandleModel::~DuiAppletHandleModel()
{
}
...
QString Ut_DuiAppletInstanceManager::callInterface;
QList<QString> Ut_DuiAppletInstanceManager::callMethods;
QList< QList<QVariant> > Ut_DuiAppletInstanceManager::callArguments;
bool Ut_DuiAppletInstanceManager::callError = false;
QString Ut_DuiAppletInstanceManager::replyDataString;

// AppletInstanceCollection (connected to DuiAppletInstanceManager)
...
DuiWidget *AppletInstanceCollection::at(int index) const
{
    return instances.at(index);
}

void AppletInstanceCollection::addInstance(DuiWidget *widget, DuiDataStore &store)
{
    Q_UNUSED(store);
    instances.append(widget);
}

void AppletInstanceCollection::removeInstance(DuiWidget *widget)
{
...
// DuiAppletMetaData stubs (used by DuiAppletInstanceManager)
...
// QProcess stubs (used by DuiAppletHandle)
...
{
    // These are used by DuiDesktopEntry and DuiAppletMetaData
    if (key == "Desktop Entry/Type")
        return QString("DUIApplet");
        ...
        return QString(outOfProcess ? "duiappletrunner" : "");
...
bool DuiFileDataStore_isReadable = true;
bool DuiFileDataStore::isReadable() const
{
    return DuiFileDataStore_isReadable;
}
bool DuiFileDataStore_isWritable = true;
bool DuiFileDataStore::isWritable() const
{
    return DuiFileDataStore_isWritable;
}

QMap<QString, QVariant> instantiateAppletPlaceHolderMetaData;

// QFile stubs (used by DuiAppletInstanceManager)
...
// QDir stubs (used by DuiAppletInstanceManager)
...
// QFileInfo stubs (used by DuiAppletMetaData)
...
DuiTestAppletInstanceManager::DuiTestAppletInstanceManager(const QString &identifier, DuiDataStore *dataStore) :
    DuiAppletInstanceManager(identifier, dataStore)
{
}
...
void Ut_DuiAppletInstanceManager::init()
{
    ...
    manager = new DuiTestAppletInstanceManager("testmanager", defaultDataStore);
    ...
    DuiFileDataStore_isReadable = true;
    DuiFileDataStore_isWritable = true;
...
void Ut_DuiAppletInstanceManager::cleanup()
{
    // Destroy the manager
    delete manager;
    manager = 0;
...
void Ut_DuiAppletInstanceManager::testInProcessAppletRestoration()
{
    ...
    QCOMPARE(gDuiAppletLoaderStub->stubCallCount("loadApplet"), 2);
    QCOMPARE(gDuiAppletLoaderStub->stubCallsTo("loadApplet").at(0)->parameter<QString>(0), QString("/tmp/testapplet1.desktop"));
    QCOMPARE(gDuiAppletLoaderStub->stubCallsTo("loadApplet").at(1)->parameter<QString>(0), QString("/tmp/testapplet4.desktop"));
}

void Ut_DuiAppletInstanceManager::testOutOfProcessAppletRestoration()
{
    outOfProcess = true;

    // Create AppletInstanceCollection to verify created applet instances
    AppletInstanceCollection collection;
    QObject::connect(manager, SIGNAL(appletInstantiated(DuiWidget *, DuiDataStore &)), &collection, SLOT(addInstance(DuiWidget *, DuiDataStore &)));
    ...
    DuiAppletHandle *instance1 = dynamic_cast<DuiAppletHandle *>(collection.at(0));
    DuiAppletHandle *instance2 = dynamic_cast<DuiAppletHandle *>(collection.at(1));
    QVERIFY(instance1 != NULL);
    QVERIFY(instance2 != NULL);
    QList<MethodCall *> initCalls = gDuiAppletHandleStub->stubCallsTo("init");
    QCOMPARE(initCalls.length(), 2);
    QCOMPARE(initCalls[0]->parameter<const DuiAppletId &>(3).toString(), QString("ut_duiappletinstancemanager/testmanager/1"));
    QCOMPARE(initCalls[1]->parameter<const DuiAppletId &>(3).toString(), QString("ut_duiappletinstancemanager/testmanager/4"));

    // Make sure the titles are set
    QList<MethodCall *> setAppletTitleCalls = gDuiAppletHandleStub->stubCallsTo("setAppletTitle");
...
void Ut_DuiAppletInstanceManager::testAppletDataStoring()
{
    // Create DuiAppletInstanceManager with a mock datastore.
    MockDataStore mockDataStore;
    delete manager;
    manager = new DuiTestAppletInstanceManager("testmanager", &mockDataStore);
...
void Ut_DuiAppletInstanceManager::testAppletInstantiation()
{
    // Create DuiAppletInstanceManager with a mock datastore.
    MockDataStore mockDataStore;
    delete manager;
...
void Ut_DuiAppletInstanceManager::testAppletInstantiationWithMissingDesktopFiles()
{
    // Create DuiAppletInstanceManager with a mock datastore.
    existingFiles.clear();
    MockDataStore mockDataStore;
...
void Ut_DuiAppletInstanceManager::testAppletInstanceRemoval()
{
    QSignalSpy instantiateSpy(manager, SIGNAL(appletInstantiated(DuiWidget *, DuiDataStore &)));
    QSignalSpy removeSpy(manager, SIGNAL(appletRemoved(DuiWidget *)));

    // Verify that the instantiation process does not return an error
    QVERIFY(manager->restoreApplets());

    // Remove one of the instantiated applets
    DuiAppletId appletId("ut_duiappletinstancemanager", "testmanager", 4);
    ...
    QString dataFileName = QDir::homePath() + "/.config/ut_duiappletinstancemanager/testmanager/4.data";
    QCOMPARE(removedFiles.at(0), dataFileName);

    // Verify that the applet instance settings were removed
    QCOMPARE(gDuiAppletSettingsStub->stubCallCount("removeInstanceSettingValues"), 1);
    QCOMPARE(gDuiAppletSettingsStub->stubLastCallTo("duiAppletSettingsConstructor").parameter<QString>(0), QString("/tmp/testapplet4.desktop"));
    QCOMPARE(gDuiAppletSettingsStub->stubLastCallTo("duiAppletSettingsConstructor").parameter<DuiAppletId>(1).toString(), appletId.toString());
}

void Ut_DuiAppletInstanceManager::testAppletUninstallation()
{
    QSignalSpy removeSpy(manager, SIGNAL(appletRemoved(DuiWidget *)));
    ...
    // Verify that applet instance data file was removed
    QCOMPARE(removedFiles.count(), 1);
    QString dataFileName = QDir::homePath() + "/.config/ut_duiappletinstancemanager/testmanager/1.data";
    QCOMPARE(removedFiles.at(0), dataFileName);
}

void Ut_DuiAppletInstanceManager::testAppletInstanceDataStoreIsNonReadWrite()
{
    // Create DuiAppletInstanceManager with a mock datastore.
    MockDataStore mockDataStore;
    delete manager;
    ...
    DuiFileDataStore_isReadable = false;
    QVERIFY(!manager->instantiateApplet("/tmp/testapplet1.desktop"));
    QVERIFY(!mockDataStore.contains("1/desktopFile"));
    QCOMPARE(spy.count(), 0);

    DuiFileDataStore_isWritable = false;
    QVERIFY(!manager->instantiateApplet("/tmp/testapplet1.desktop"));
    QVERIFY(!mockDataStore.contains("1/desktopFile"));
    QCOMPARE(spy.count(), 0);
...
void Ut_DuiAppletInstanceManager::testPreconfiguredAppletInstances()
{
    existingFiles.append(DUI_XDG_DIR "/" "ut_duiappletinstancemanager/testmanager.data");

    delete manager;
    manager = new DuiTestAppletInstanceManager("testmanager");
    QCOMPARE(dirMkpath, QDir::homePath() + "/.config/ut_duiappletinstancemanager/");
    QCOMPARE(fileCopy.first,  QString(DUI_XDG_DIR "/" "ut_duiappletinstancemanager/testmanager.data"));
    QCOMPARE(fileCopy.second, QDir::homePath() + "/.config/ut_duiappletinstancemanager/testmanager.data");
    ...
    dirMkpath = QString();
    dataDirectoryExists = false;
    existingFiles.append(QDir::homePath() + "/.config/ut_duiappletinstancemanager/testmanager.data");
    delete manager;
    manager = new DuiTestAppletInstanceManager("testmanager");
    QCOMPARE(dirMkpath, QString());
...
void Ut_DuiAppletInstanceManager::testInstallingAppletFromPackage()
{
    // Create a signal spy to investigate if appletInstantiated signal is emitted
    QSignalSpy spy(manager, SIGNAL(appletInstantiated(DuiWidget *, DuiDataStore &)));
    ...
    QCOMPARE(gDuiAppletHandleStub->stubCallCount("initPlaceHolder"), 1);
    QCOMPARE(gDuiAppletHandleStub->stubLastCallTo("initPlaceHolder").parameter<const DuiAppletId &>(0).instanceId(), (uint)2);
...
void Ut_DuiAppletInstanceManager::testInstallationOperationComplete()
{
    // Instantiate an applet from a package
    QMap<QString, QVariant> metaData;
    ...
    QCOMPARE(gDuiAppletHandleStub->stubCallCount("init"), 1);
    QCOMPARE(gDuiAppletHandleStub->stubLastCallTo("init").parameter<const DuiAppletId &>(3).instanceId(), (uint)2);
}

void Ut_DuiAppletInstanceManager::testInstallationOperationCompleteForUnknownPackage()
{
    // Instantiate an applet from a package
    QMap<QString, QVariant> metaData;
    metaData.insert("Maemo-Desktop-File", "/tmp/test.desktop");
    ...
    QCOMPARE(gDuiAppletHandleStub->stubCallCount("init"), 0);
}

void Ut_DuiAppletInstanceManager::testUnknownOperationComplete()
{
    // Instantiate an applet from a package
    QMap<QString, QVariant> metaData;
    metaData.insert("Maemo-Desktop-File", "/tmp/test.desktop");
...
void Ut_DuiAppletInstanceManager::testRestoringAppletsBeingInstalled()
{
    // Verify that the restoration process does not return an error
    captureCalls = true;
    QVERIFY(manager->restoreApplets());
...
void Ut_DuiAppletInstanceManager::testReceiveOperationFailed()
{
    QSignalSpy spy(manager, SIGNAL(appletInstantiated(DuiWidget *, DuiDataStore &)));
    connect(this, SIGNAL(receiveOperation(QDBusPendingCallWatcher *)), manager, SLOT(receiveOperation(QDBusPendingCallWatcher *)));

    DuiAppletId appletId("ut_duiappletinstancemanager", "testmanager", 5);
    QDBusMessage msg;
    ...
    QCOMPARE(gDuiAppletHandleStub->stubCallCount("initPlaceHolder"), 1);
    QCOMPARE(gDuiAppletHandleStub->stubLastCallTo("initPlaceHolder").parameter<const DuiAppletId &>(0).instanceId(), (uint)5);
    QCOMPARE(gDuiAppletHandleStub->stubLastCallTo("initPlaceHolder").parameter<QString>(2), qtTrId("duiappletinstancedata_nopackagemetadata"));
}

void Ut_DuiAppletInstanceManager::testReceiveUnknownOperation()
{
    QSignalSpy spy(manager, SIGNAL(appletInstantiated(DuiWidget *, DuiDataStore &)));
    connect(this, SIGNAL(receiveOperation(QDBusPendingCallWatcher *)), manager, SLOT(receiveOperation(QDBusPendingCallWatcher *)));
...
void Ut_DuiAppletInstanceManager::testReceiveInstallOperation()
{
    QSignalSpy spy(manager, SIGNAL(appletInstantiated(DuiWidget *, DuiDataStore &)));
    connect(this, SIGNAL(receiveOperation(QDBusPendingCallWatcher *)), manager, SLOT(receiveOperation(QDBusPendingCallWatcher *)));
    ...
    QCOMPARE(gDuiAppletHandleStub->stubCallCount("initPlaceHolder"), 1);
    QCOMPARE(gDuiAppletHandleStub->stubLastCallTo("initPlaceHolder").parameter<const DuiAppletId &>(0).instanceId(), (uint)5);
    QCOMPARE(gDuiAppletHandleStub->stubLastCallTo("initPlaceHolder").parameter<QString>(2), QString());
}

QTEST_APPLESS_MAIN(Ut_DuiAppletInstanceManager)
