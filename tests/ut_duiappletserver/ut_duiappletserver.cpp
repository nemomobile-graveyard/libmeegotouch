...
void Ut_DuiAppletServer::init()
{
    connected = false;

    m_subject = new DuiAppletServer;
    connect(m_subject, SIGNAL(connectionEstablished()), this, SLOT(connectionEstablished()));
}

void Ut_DuiAppletServer::cleanup()
{
    delete socket;
    socket = NULL;
    delete m_subject;
...
void Ut_DuiAppletServer::setupServer()
{
    // Initiate connection to a process
    QCOMPARE(m_subject->startServer("duioopwidget_1"), true);

    // Emulate being the process to connect to
    socket = new QLocalSocket;
    socket->connectToServer("duioopwidget_1");
...
void Ut_DuiAppletServer::testConnection()
{
    setupServer();
...
void Ut_DuiAppletServer::testReconnect()
{
    setupServer();
...
void Ut_DuiAppletServer::connectionEstablished()
{
    connected = true;
}

QTEST_MAIN(Ut_DuiAppletServer)
