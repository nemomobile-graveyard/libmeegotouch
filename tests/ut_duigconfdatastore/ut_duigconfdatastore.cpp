...
#include "ut_duigconfdatastore.h"
...
QMultiHash<QString, DuiGConfItem *> gDuiGConfItems;
QList<QString> gUnsetDuiGconfItems;
    ...
    : QObject(parent)
{
    ...
    gDuiGConfItems.insert(key, this);
}

DuiGConfItem::~DuiGConfItem()
{
    gDuiGConfItems.remove(priv->key, this);
    delete priv;
}
...
{
    gUnsetDuiGconfItems.append(priv->key);
    set(QVariant());
}

// Test class
void Ut_DuiGConfDataStore::initTestCase()
{
}

void Ut_DuiGConfDataStore::cleanupTestCase()
{
}

void Ut_DuiGConfDataStore::init()
{
    stringVariantSlotArguments.clear();
    gDuiGConfItems.clear();
    gUnsetDuiGconfItems.clear();

    m_subject = new DuiGConfDataStore("/gconf");
}

void Ut_DuiGConfDataStore::cleanup()
{
    delete m_subject;
    m_subject = NULL;

    // Check after deleting the test subject that it didn't leave any memory reserved
    QCOMPARE(gDuiGConfItems.count(), 0);
}

void Ut_DuiGConfDataStore::testReading()
{
    QCOMPARE(m_subject->value("key").toString(), QString("value"));
}

void Ut_DuiGConfDataStore::testWriting()
{
...
void Ut_DuiGConfDataStore::testReadingNonExistingKey()
{
...
void Ut_DuiGConfDataStore::testWritingNonExistingKey()
{
...
void Ut_DuiGConfDataStore::testSignalReceivedWhenValueChanges()
{
    // Set a new value for the item
    DuiGConfItem *gconfItem = gDuiGConfItems.value("/gconf/key");
    ...
    gconfItem = gDuiGConfItems.value("/gconf/key1");
...
void Ut_DuiGConfDataStore::stringVariantSlot(const QString &string, const QVariant &variant)
{
    stringVariantSlotArguments.append(qMakePair(string, variant));
}

void Ut_DuiGConfDataStore::testAllKeys()
{
...
void Ut_DuiGConfDataStore::testRemove()
{
...
void Ut_DuiGConfDataStore::testClear()
{
    QCOMPARE(gDuiGConfItems.count(), 3);
    m_subject->clear();
    QCOMPARE(gDuiGConfItems.count(), 0);
}

void Ut_DuiGConfDataStore::testContains()
{
    QVERIFY(m_subject->contains("key"));
    QVERIFY(!m_subject->contains("bar"));
}

void Ut_DuiGConfDataStore::testSubPathsNotSupported()
{
...
QTEST_APPLESS_MAIN(Ut_DuiGConfDataStore)
