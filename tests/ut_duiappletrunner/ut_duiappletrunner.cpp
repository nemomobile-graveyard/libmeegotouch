...
// QTimer stubs (used by DuiAppletRunner)
void QTimer::start(int)
{
    if (Ut_DuiAppletRunner::timerImmediateTimeout) {
        emit timeout();
    }
}

// QCoreApplication stubs (used by DuiAppletRunner)
void QCoreApplication::quit()
{
    Ut_DuiAppletRunner::quitCalled = true;
}

// DuiSettingsLanguageBinary stubs (used by DuiAppletRunner)
QStringList DuiSettingsLanguageBinary::keys() const
{
    QStringList list;
    if (Ut_DuiAppletRunner::processingInstanceSettingsFile) {
        list.append("instanceKey1");
        list.append("instanceKey2");
    } else if (Ut_DuiAppletRunner::processingGlobalSettingsFile) {
        list.append("globalKey1");
        list.append("globalKey2");
    }
    return list;
}

// QFile stubs (used by DuiAppletRunner)
bool QFile::exists() const
{
    Ut_DuiAppletRunner::processingInstanceSettingsFile = fileName().contains("-instance.xml");
    Ut_DuiAppletRunner::processingGlobalSettingsFile = fileName().contains("-global.xml");
    return true;
}

// DuiSettingsLanguageParser stubs (used by DuiAppletRunner)
...
bool DuiAppletLoader_doConstructWidget = true;
bool DuiAppletLoader_returnPreCreatedWidget = false;
DuiWidget *widget;
DuiWidget *DuiAppletLoader::loadApplet(const DuiAppletMetaData &, DuiDataStore &, DuiDataAccess &)
{
    if (DuiAppletLoader_doConstructWidget) {
        if (DuiAppletLoader_returnPreCreatedWidget) {
            return widget;
        } else {
            DuiWidget *widget = new DuiWidget;
            widget->setMinimumSize(QSizeF(50, 50));
            widget->setPreferredSize(QSizeF(100, 100));
            widget->setMaximumSize(QSizeF(150, 150));
            return widget;
        }
    } else {
        return NULL;
...
bool Ut_DuiAppletRunner::timerImmediateTimeout;
bool Ut_DuiAppletRunner::quitCalled;
bool Ut_DuiAppletRunner::processingInstanceSettingsFile;
bool Ut_DuiAppletRunner::processingGlobalSettingsFile;
...
void Ut_DuiAppletRunner::init()
{
    ...
    meta = new DuiAppletMetaData("ut_duiappletrunner/ut_duiappletrunner.desktop");
    ...
    processingInstanceSettingsFile = false;
    processingGlobalSettingsFile = false;
    ...
    DuiAppletMetaData_isValidReturn = true;
    DuiAppletLoader_doConstructWidget = true;
    sceneRenderCalled = false;
    DuiAppletLoader_returnPreCreatedWidget = false;
    widget = NULL;
...
{
    QCOMPARE(m_instance->init("appletId", "/tmp/applet.data", *meta, "servername"), true);
    QCOMPARE(gDuiAppletSharedMutexStub->stubLastCallTo("init").parameter<QString>(0), QString("servername"));
    QCOMPARE(gDuiAppletSettingsStub->stubLastCallTo("duiAppletSettingsConstructor").parameter<QString>(0), QString("ut_duiappletrunner/ut_duiappletrunner.desktop"));
...
{
    QCOMPARE(m_instance->init("", "/tmp/applet.data", *meta, "servername"), false);
}

void Ut_DuiAppletRunner::testInitializationFailInvalidInstanceDataLocation()
{
    QCOMPARE(m_instance->init("appletId", "", *meta, "servername"), false);
}

void Ut_DuiAppletRunner::testInitializationFailInvalidMetadata()
{
    DuiAppletMetaData_isValidReturn = false;
    QCOMPARE(m_instance->init("appletId", "/tmp/applet.data", *meta, "servername"), false);
}

void Ut_DuiAppletRunner::testInitializationFailCommunicationFails()
{
...
{
    DuiAppletLoader_doConstructWidget = false;
    QCOMPARE(m_instance->init("appletId", "/tmp/applet.data", *meta, "servername"), false);
}

void Ut_DuiAppletRunner::testInitializationFailSharedMemoryAttachFails()
{
    gDuiAppletSharedMutexStub->stubSetReturnValue("init", false);
    QCOMPARE(m_instance->init("appletId", "/tmp/applet.data", *meta, "servername"), false);
}

void Ut_DuiAppletRunner::testRelayingMouseEvents()
{
    m_instance->init("appletId", "/tmp/applet.data", *meta, "servername");
...
void Ut_DuiAppletRunner::testAppletRunnerQuitsIfMessageReceivingTimeouts()
{
    m_instance->init("appletId", "/tmp/applet.data", *meta, "servername");
...
void Ut_DuiAppletRunner::testMessageReceivedBeforeAppletInit()
{
    {
...
void Ut_DuiAppletRunner::testPaintingOnPixmap()
{
...
void Ut_DuiAppletRunner::testSendUpdateGeometryMessage()
{
    m_instance->init("appletId", "/tmp/applet.data", *meta, "servername");
...
void Ut_DuiAppletRunner::testSceneChanged()
{
    connect(this, SIGNAL(changeScene(QList<QRectF>)), m_instance, SLOT(sceneChanged(QList<QRectF>)));
...
void Ut_DuiAppletRunner::testObjectMenuRequestMessageGetsPropagated()
{
