...
#include "ut_duiappletcommunicator.h"
...
// static variables
TestAppletCommunicator *Ut_DuiAppletCommunicator::m_subject;
...
public:
    virtual qint64 bytesAvailable() const {
        return Ut_DuiAppletCommunicator::m_subject->getStream().device()->bytesAvailable();
    }
};
...
public:
    ...
    TestAppletMessage(uint value = 2, DuiAppletMessageType type = (DuiAppletMessageType)1) :
...
void myMessageSerializer(QDataStream &stream, const DuiAppletMessage &message)
{
...
// The test class
void Ut_DuiAppletCommunicator::initTestCase()
{
}

void Ut_DuiAppletCommunicator::cleanupTestCase()
{
}

void Ut_DuiAppletCommunicator::init()
{
    ...
    connect(m_subject, SIGNAL(messageReceived(DuiAppletMessage)), this, SLOT(messageReceived(DuiAppletMessage)));
}

void Ut_DuiAppletCommunicator::cleanup()
{
    delete m_subject;
    m_subject = NULL;
}

void Ut_DuiAppletCommunicator::testIsConnected()
{
...
void Ut_DuiAppletCommunicator::testMessageSendingWhenNotConnected()
{
...
void Ut_DuiAppletCommunicator::testMessageSending()
{
...
void Ut_DuiAppletCommunicator::testReceivingMessages()
{
    m_subject->setupConnection();
...
void Ut_DuiAppletCommunicator::testReceivingMessageWithSocketFailure()
{
    m_subject->setupConnection();
...
void Ut_DuiAppletCommunicator::testInvalidMessageTypeReceiving()
{
    m_subject->setupConnection();

    // Put test data with unknown message type to the stream
    myMessageSerializer(m_subject->getStream(), TestAppletMessage(2, (DuiAppletMessage::DuiAppletMessageType)10));
    m_subject->getStream().device()->seek(0);
...
void Ut_DuiAppletCommunicator::testReceivingMessageInMultipleParts()
{
    m_subject->setupConnection();
...
void Ut_DuiAppletCommunicator::messageReceived(const DuiAppletMessage &message)
{
    if (message.type() == 1) {
...
QTEST_APPLESS_MAIN(Ut_DuiAppletCommunicator)
