...
EchoWindow::EchoWindow()
{
    ...
    textProcessorIf = new TextProcessorIf();

    // check the interface is valid
    if (!textProcessorIf->isValid()) {
    ...
    if (textProcessorIf->isValid()) {
        QStringList services = textProcessorIf->serviceNames();
    ...
    if (textProcessorIf->isValid()) {
        QStringList services = textProcessorIf->serviceNames("com.nokia.TextProcessorIf");
        QStringListIterator iterator(services);
        qDebug() << "Services implementing com.nokia.TextProcessorIf:";
        while (iterator.hasNext())
            qDebug() << iterator.next().toLocal8Bit().constData();
    }
    ...
    connect(
        textProcessorIf, SIGNAL(serviceAvailable(QString)),
        this, SLOT(enableTextProcessing(QString))
    );
    connect(
        textProcessorIf, SIGNAL(serviceUnavailable(QString)),
        this, SLOT(disableTextProcessing(QString))
    );
    connect(
        textProcessorIf, SIGNAL(serviceChanged(QString)),
        this, SLOT(enableTextProcessing(QString))
    );
}
...
EchoWindow::~EchoWindow()
{
    delete textProcessorIf;
}

void EchoWindow::sendEcho()
{
    // call the interface method to reverse the string
    QString text = textProcessorIf->reverse(lineEdit->text());
