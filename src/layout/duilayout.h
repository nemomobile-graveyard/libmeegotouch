 ...
 * multiple \link DuiAbstractLayoutPolicy layout policies \endlink .
 * \ingroup layouts
 *
 ...
 * DuiLayoutAnimation.
 *
 * The DuiLayout owns all its associated policies, deleting them when the layout is deleted.
 *
 * \section relation_to_qgraphicslayout Relation to QGraphicsLayout
 * Use a DuiLayout if you want the ability to:
 *
 * \li Switch between multiple policies
 * \li Animate the position and opacity of items
 * \li Use one of the policies with no equivalent in QGraphicsLayout (e.g. DuiFlowLayoutPolicy, DuiFreestyleLayoutPolicy)
 *
 ...
 * items are added directly to the layout, however with DuiLayout items are added to the layout's
 * associated policies.  The policies all inherit from QAbstractLayoutPolicy and take care of
 * geometry management for a set of widgets.  To create more complex layouts you can nest DuiLayouts
 * inside each other or a mix of DuiLayouts and other QGraphicsLayouts.
 *
 * The following code creates a DuiLayout with a linear and a flow policy, adding items to both policies.
 * The first policy created will be the default policy.  The other policies can be switched to by calling
 * DuiAbstractLayoutPolicy::activate().
 *
 ...
 *
 * Another layout (DuiLayout or QGraphicsLayout) can be added to a policy to nest layouts to create
 ...
 * they are in the currently active policy.  If your widget inherits DuiWidget then you can still
 * manually hide an item by calling DuiWidget::hide().  Usually it is sufficient to just call
 * <code>mywidget->hide()</code>, but sometimes you have a QGraphicsItem or QGraphicsLayoutItem pointer
 * that you must first cast to a DuiWidget.
 *
 ...
 *
 * CSS styling is done on the policies themselves - see DuiAbstractLayoutPolicy for more information.
 *
 ...
 *
 * If you are implementing a custom DuiWidgetController and DuiWidgetView, then implement
 * DuiWidgetView::sizeHint(), leaving the default DuiWidgetController::sizeHint() function
 * which just calls the view's \link DuiWidgetView::sizeHint() sizeHint \endlink function.
 *
 * For an item in the layout, the QGraphicsLayoutItem::sizePolicy() function determines
 * how the item should expand when there is space available.  For example, a DuiButton cannot
 ...
 */
class DUI_EXPORT DuiLayout : public QGraphicsLayout
{
public:
     ...
     */
    explicit DuiLayout(QGraphicsLayoutItem *parent = 0);
    ...
    /*!
     * \deprecate Policies should use DuiAbstractLayoutPolicy::invalidate instead. Since 0.19
     ...
     *
     * This just calls the \link DuiAbstractLayoutPolicy::sizeHint() sizeHint function\endlink for the current policy.
     */
     ...
     *
     * This is equivalent to calling DuiAbstractLayoutPolicy::activate().
     *
     * @param policy  The policy to use for all layout operations.
     */
    void setPolicy(DuiAbstractLayoutPolicy *policy);
     ...
     */
    void setLandscapePolicy(DuiAbstractLayoutPolicy *policy);

    /*! \brief The policy to be used in landscape mode.
     *
     * The default is NULL - the policy is not changed when the device is rotated.
     */
    DuiAbstractLayoutPolicy *landscapePolicy() const;
     ...
     */
    void setPortraitPolicy(DuiAbstractLayoutPolicy *policy);

    /*! \brief The policy to be used in portrait mode.
     *
     * The default is NULL - the policy is not changed when the device is rotated.
     */
    DuiAbstractLayoutPolicy *portraitPolicy() const;
     ...
     */
    DuiAbstractLayoutPolicy *policy() const;
     ...
     */
    QList<DuiAbstractLayoutPolicy *> registeredPolicies() const;
     ...
     * \sa animation()
     */
    void setAnimation(DuiLayoutAnimation *animation);
     ...
     * \sa setAnimation()
     */
    DuiLayoutAnimation *animation() const;
     ...
     * policies.
     *
     * If the current \link DuiAbstractLayoutPolicy::getContentsMargins() policy's getContentsMargins() \endlink
     ...
     *
     * \sa getContentsMargins(), DuiAbstractLayoutPolicy::setContentsMargins(), DuiAbstractLayoutPolicy::getContentsMargins()
     */
     ...
     * current \link DuiAbstractLayoutPolicy::getContentsMargins() policy's getContentsMargins()\endlink, unless that
     ...
     * @param bottom Bottom margin.
     *
     * \sa setContentsMargins(), DuiAbstractLayoutPolicy::setContentsMargins(), DuiAbstractLayoutPolicy::getContentsMargins()
     */
...
protected:
     ...
     *
     * This is called by DuiAbstractLayoutPolicy when an item is added to the policy.
     *
     * For a user of DuiLayout, this function is protected.  To add items to the layout,
     ...
     *
     * \code
     *   QGraphicsLayout *layout = someDuiLayout;
    ...
    /** \internal
     * Private pointer */
    DuiLayoutPrivate *const d_ptr;
    /** \internal_end */

private:
    Q_DISABLE_COPY(DuiLayout)
    Q_DECLARE_PRIVATE(DuiLayout)


    friend class DuiAbstractLayoutPolicy;
    friend class DuiLayoutAnimation;
    friend class DuiBasicLayoutAnimationPrivate;
    friend class Ut_DuiLayout;
};
