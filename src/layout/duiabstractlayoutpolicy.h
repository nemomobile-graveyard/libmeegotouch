...
#ifndef DUIABSTRACTLAYOUTPOLICY_H
#define DUIABSTRACTLAYOUTPOLICY_H
 ...
 * \brief The DuiAbstractLayoutPolicy class provides the base class for all DuiLayout policies.
 * \ingroup layouts
 *
 * DuiAbstractLayoutPolicy is an abstract class that defines an API for
 * arranging QGraphicsWidget, DuiWidget and other QGraphicsLayoutItem objects
 * in the associated DuiLayout.
 *
 * A DuiLayout can have many different policies, with one of the policies being
 * active at any moment.  Changing a DuiLayout's active policy causes the items
 ...
 * manually hide an item by calling DuiWidget::hide().

 * \section multiple_policies Multiple Policies
 *
 * A layout can have multiple policies associated with it.  See the DuiLayout documentation for more information and examples.
 *
 * \sa DuiLayout, \ref layout-multiplepolicies, \ref layout-inside-layout
 *
 ...
 * DuiAbstractLayoutPolicy.  Their object name can be set with setObjectName() and they
 * can be styled different depending on the current device orientation.
 *
 * For example:
 *
 * \code
 *  DuiLinearLayoutPolicy mypolicy(mylayout);
 ...
 * <tr>
 *    <td>DuiAbstractLayoutPolicy::addItem()</td>
...
class DUI_EXPORT DuiAbstractLayoutPolicy
{
public:
     ...
     */
    explicit DuiAbstractLayoutPolicy(DuiLayout *layout);
     ...
     * policy continue to be owned by the associated DuiLayout and are
     * \link QGraphicsItem::hide() hidden\endlink if not in the new policy.
     *
     * \sa activate()
     */
    virtual ~DuiAbstractLayoutPolicy();
     ...
     *
     * By default, these are all -1, meaning that the DuiLayout::getContentsMargins()
     * will be used.  Any that are changed to be a non-negative number will override the
     * value returned by DuiLayout::getContentsMargins()
     *
     * @param left Left margin.  Set to -1 indicate to use DuiLayout's left margin
     * @param top Top margin.  Set to -1 indicate to use DuiLayout's top margin
     * @param right Right margin.  Set to -1 indicate to use DuiLayout's right margin
     * @param bottom Bottom margin.  Set to -1 indicate to use DuiLayout's bottom margin
     *
     * To use the values set in the CSS again, call unsetContentsMargins();
     *
     * \sa DuiLayout::getContentsMargins()
     * \sa DuiLayout::setContentsMargins()
     ...
     *
     * By default, these are all -1, meaning that the DuiLayout's getContentsMargins
     * will be used.  Any that are changed to be a non-negative number will override the
     * value returned by DuiLayout's getContentsMargins
     *
     * @param left Left margin.  -1 indicates to use DuiLayout's left margin
     * @param top Top margin.  -1 indicates to use DuiLayout's top margin
     * @param right Right margin.  -1 indicates to use DuiLayout's right margin
     * @param bottom Bottom margin.  -1 indicates to use DuiLayout's bottom margin
     */
     ...
     * getContentsMargins()
     *
     * External classes, except for DuiLayout, should not call this directly,
     * but should use DuiLayout::effectiveSizeHint()
     */
     ...
     * the object name for the DuiStyle object name.
     * For example:
     * \code
     *  DuiLinearLayoutPolicy mypolicy(mylayout);
     *  mypolicy.setObjectName("address");
     ...
     *  could then be styled in the .css file with
     * \code
     *  DuiAbstractLayoutPolicy#address {
     *    vertical-spacing: 5;
     *  }
     * \endcode
     *
     ...
     * and the item is a QGraphicsWidget (or DuiWidget) then the item will be hidden
...
protected:
     ...
     */
    explicit DuiAbstractLayoutPolicy(DuiAbstractLayoutPolicyPrivate &);
    ...
    const DuiAbstractLayoutPolicyStyle *style() const;
     ...
     * This is called whenever the layout itself is invalidated by DuiLayout, and also called by
    ...
    // shared d_ptr:
    DuiAbstractLayoutPolicyPrivate *const d_ptr;

private:
    Q_DISABLE_COPY(DuiAbstractLayoutPolicy)
    Q_DECLARE_PRIVATE(DuiAbstractLayoutPolicy)

    friend class DuiLayout;
