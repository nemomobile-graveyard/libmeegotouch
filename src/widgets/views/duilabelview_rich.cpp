...
static const int DUI_HIGHLIGHT_PROPERTY       = QTextFormat::UserProperty;
static const int DUI_HIGHLIGHTER_ID_PROPERTY  = QTextFormat::UserProperty + 1;

DuiLabelViewRich::DuiLabelViewRich(DuiLabelViewPrivate *viewPrivate) :
    DuiLabelViewSimple(viewPrivate), textDocumentDirty(true), mouseDownCursorPos(-1)
{
    textDocument.setDocumentMargin(0);
}

DuiLabelViewRich::~DuiLabelViewRich()
{
}


QPixmap DuiLabelViewRich::generatePixmap()
{
...
/**
  ...
  */
bool DuiLabelViewRich::shouldElide() const
{
...
void DuiLabelViewRich::ensureDocumentIsReady()
{
...
bool DuiLabelViewRich::resizeEvent(QGraphicsSceneResizeEvent *event)
{
...
QSizeF DuiLabelViewRich::sizeHint(Qt::SizeHint which, const QSizeF &constraint) const
{
    switch (which) {
    case Qt::MinimumSize: {
...
bool DuiLabelViewRich::updateData(const QList<const char *>& modifications)
{
    ...
    foreach(member, modifications) {
        if (member == DuiLabelModel::Text) {
            ...
            viewPrivate->textOptions.setAlignment(model->alignment());
            textDocumentDirty = true;
        } else if (member == DuiLabelModel::TextElide) {
            textDocumentDirty = true;
        } else if (member == DuiLabelModel::UseModelFont || member == DuiLabelModel::Font) {
            textDocument.setTextWidth(viewPrivate->boundingRect().width());
            needUpdate = true;
            textDocumentDirty = true;
        } else if (member == DuiLabelModel::Highlighters) {
            textDocumentDirty = true;
        }
    }
...
bool DuiLabelViewRich::isRich()
{
    return true;
}

void DuiLabelViewRich::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    event->ignore();
    ...
    if (cursorPos >= 0) {
        ...
        if (format.boolProperty(DUI_HIGHLIGHT_PROPERTY)) {
            event->accept();
...
void DuiLabelViewRich::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    int cursorPos = textDocument.documentLayout()->hitTest(event->pos() - pixmapOffset, Qt::ExactHit);
    if (cursorPos >= 0) {
        QTextCursor cursor(&textDocument);
        ...
        if (format.boolProperty(DUI_HIGHLIGHT_PROPERTY)) {
            int idx = cursor.charFormat().intProperty(DUI_HIGHLIGHTER_ID_PROPERTY);
...
void DuiLabelViewRich::cancelEvent(DuiCancelEvent *event)
{
    Q_UNUSED(event);
...
void DuiLabelViewRich::longPressEvent(QGraphicsSceneContextMenuEvent *event)
{
    int cursorPos = textDocument.documentLayout()->hitTest(event->pos() - pixmapOffset, Qt::ExactHit);
    if (cursorPos >= 0) {
        QTextCursor cursor(&textDocument);
 ...
 */
int DuiLabelViewRich::cursorPositionOfLastVisibleCharacter()
{
...
void DuiLabelViewRich::updateRichTextEliding()
{
...
void DuiLabelViewRich::updateHighlighting()
{
    //TODO: Should the highlight format come from the highlighter object?

    DuiLabelHighlighterList list = viewPrivate->model()->highlighters();
    const int listSize = list.size();
    for (int i = 0; i < listSize; ++i) {
        const DuiLabelHighlighter *highlighter = list[i];
        QTextCursor cursor(&textDocument);
        while (!cursor.isNull() && !cursor.atEnd()) {
            ...
            if (!cursor.isNull()) {
                ...
                if (highlighter->validate(item)) {
                    ...
                    format.setProperty(DUI_HIGHLIGHT_PROPERTY, true);
                    format.setProperty(DUI_HIGHLIGHTER_ID_PROPERTY, i);
...
QString DuiLabelViewRich::wrapTextWithSpanTag(const QString &text) const
{
    QString spanTag("<span>%1</span>");
    return spanTag.arg(text);
}

void DuiLabelViewRich::applyStyle()
{
    textDocumentDirty = true;
    ensureDocumentIsReady();
}
