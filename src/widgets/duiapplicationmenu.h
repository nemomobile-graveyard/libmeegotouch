...
#ifndef DUIAPPLICATIONMENU_H
#define DUIAPPLICATIONMENU_H

#include <duiscenewindow.h>
#include <duiapplicationmenumodel.h>

class DuiApplicationMenuPrivate;

/*!
    \class DuiApplicationMenu
    \brief DuiApplicationMenu widget shows a menu containing action commands.

    \ingroup widgets

    \section DuiApplicationMenuOverview Overview
        Application menu is a menu element that contains \link DuiAction \endlink and
        \link DuiWidgetAction \endlink commands which get executed on activation. It consists
        ...
        DuiAction * action = new DuiAction("Pictures", this);
        action->setLocation(DuiAction::ApplicationMenuLocation);
        menu->addAction(action);

        //adding a style action
        DuiAction * styleAction = new DuiAction("", this);
        styleAction->setLocation(DuiAction::ApplicationMenuLocation);
    ...
    \section DuiMenuUseGuidelines Usage guidelines
    ...
    \section DuiApplicationMenuVariants Variants
        \li \link DuiApplicationMenuView Default application menu view \endlink

    \sa DuiApplicationMenuModel DuiApplicationMenuStyle

*/

class DUI_EXPORT DuiApplicationMenu : public DuiSceneWindow
{
    Q_OBJECT
    DUI_CONTROLLER(DuiApplicationMenu)
...
public:
    ...
    */
    explicit DuiApplicationMenu(const QString &viewType = QString());

    /*!
        \brief Destructs menu
    */
    virtual ~DuiApplicationMenu();
...
protected:
    ...
    Q_DECLARE_PRIVATE(DuiApplicationMenu)

private:
    Q_DISABLE_COPY(DuiApplicationMenu)

    friend class DuiApplicationMenuView;
    friend class DuiMenuObjectView;

#ifdef UNIT_TEST
    friend class Pt_DuiApplicationMenu;
    friend class Ut_DuiApplicationMenu;
#endif
