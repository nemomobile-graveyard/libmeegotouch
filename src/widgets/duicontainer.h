...
#ifndef DUICONTAINER_H
#define DUICONTAINER_H
...
class DuiContainerPrivate;

/*!
 * \class DuiContainer
 * \brief A DuiContainer is a generic container which can be used for any DuiWidgets.
 *
 * \section DuiContainerOverview Overview
 ...
 *  \code
 *
 * DuiContainer *box = new DuiContainer();
 *
 * box->setTitle("Header");
 * box->setText("More...");
 ...
 *  \endcode
 *
 * \section DuiContainerUseGuidlines Usage guidelines
 ...
 * label truncates.
 *
 * \sa DuiContainerModel, DuiContainerView, \see \ref appletdevelopment
 */

class DUI_EXPORT DuiContainer : public DuiWidgetController
{
    Q_OBJECT
...
public:
     ...
     */
    DuiContainer(QGraphicsItem *parent = 0);

    /*!
     * \overload
     * \brief Constructs a container with the title \a title and parent \a parent.
    */
    DuiContainer(const QString &title, QGraphicsItem *parent = 0);
...
private:
    Q_DECLARE_PRIVATE(DuiContainer)
    Q_DISABLE_COPY(DuiContainer)

#ifdef UNIT_TEST
    friend class Ut_DuiContainer;
#endif
};
