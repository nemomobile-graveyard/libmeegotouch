...
#ifndef DUISUBDATASTORE_H
#define DUISUBDATASTORE_H

#include "duidatastore.h"

/*!
 * DuiSubDataStore takes another DataStore object and gives a limited view to it.
 *
 * DuiSubDataStore doesn't store any data by itself but instead it uses another DataStore
 * object for this. DuiSubDataStore offers a limited view to the keys of the underlying DataStore.
 ...
 */
class DUI_EXPORT DuiSubDataStore : public DuiDataStore
{
    Q_OBJECT
    ...
    DuiDataStore &_baseStore;

public:
    /*!
     * Constructs a new DuiSubDataStore view to another DataStore object.
     * \param prefix the namespace for the DuiSubDataStore.
     * \param baseStore the underlying base DataStore object.
     */
    DuiSubDataStore(const QString &prefix, DuiDataStore &baseStore);

    /*!
     * Destructs the DuiSubDataStore.
     */
    virtual ~DuiSubDataStore();
