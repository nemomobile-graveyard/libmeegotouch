...
static const QString types[NUM_TYPES] = {
    ...
    QString("const DuiScalableImage*"),
    QString("QSize"),
    QString("QSizeF"),
    QString("QPoint"),
    QString("QPointF"),
...
Q_DECLARE_METATYPE(DuiFeedback)
...
int DuiStyleSheetAttribute::mmToInt(qreal mm)
{
    static const int pixelsPerMm = DuiDeviceProfile::instance()->pixelsPerInch().width() / 25.4;
    return mm * pixelsPerMm;
}

qreal DuiStyleSheetAttribute::mmToReal(qreal mm)
{
    static const qreal pixelsPerMm = DuiDeviceProfile::instance()->pixelsPerInch().width() / 25.4;
    return mm * pixelsPerMm;
}

int DuiStyleSheetAttribute::attributeToInt(const QString &attribute, bool *conversionOk)
{
...
int DuiStyleSheetAttribute::attributeToInt(const QString &attribute, bool *conversionOk, SizeAttributeType type, Dui::Orientation orientation)
{
    QString value = attribute.trimmed();
...
qreal DuiStyleSheetAttribute::attributeToFloat(const QString &attribute, bool *conversionOk)
{
    QString value = attribute.trimmed();
...
qreal DuiStyleSheetAttribute::attributeToFloat(const QString &attribute, bool *conversionOk, SizeAttributeType type, Dui::Orientation orientation)
{
    QString value = attribute.trimmed();
...
bool DuiStyleSheetAttribute::booleanFromString(const QString &string, bool *conversionOk)
{
...
QColor DuiStyleSheetAttribute::colorFromString(const QString &string, bool *conversionOk)
{
    QColor color(string);
    if (conversionOk)
...
QFont DuiStyleSheetAttribute::fontFromString(const QString string, bool *conversionOk)
{
        ...
        Dui::Orientation orientation)
{
    ...
    if (attributeType == types[BOOL_TYPE]) {
        ...
        //only image_id
        if (list.size() == 1) {
            const QPixmap *pixmap = DuiTheme::pixmap(list[0]);
        ...
        //image_id + width + height
            ...
            const QPixmap *pixmap = DuiTheme::pixmap(list[0], QSize(width, height));
            return property.write(style, qVariantFromValue(pixmap));
        }
        //no parameters
        else if (list.size() == 0) {
        ...
        //only image_id
            ...
            const DuiScalableImage *image = DuiTheme::scalableImage(list[0], 0, 0, 0, 0);
            return property.write(style, qVariantFromValue(image));
        }
        //image_id + border width paramaters
        else if (list.size() == 5) {
            //image_id and the border parameters
            const DuiScalableImage *image = DuiTheme::scalableImage(list[0],
    ...
    DuiStyleSheetParser::outputParseError(filename, "Not a valid attribute(" + attributeType + "): " + name + ": " + value, DuiStyleSheetParser::getLineNum(filename, position));
    return false;
}

bool DuiStyleSheetAttribute::writeAttributeShm(DuiStyle *style,
        ...
        Dui::Orientation orientation)
{
    ...
    if (containerType == REAL_TYPE) {
        ...
        const QPixmap *pixmap = DuiTheme::pixmap(name);
        return property.write(style, qVariantFromValue(pixmap));
        ...
        const QPixmap *pixmap = DuiTheme::pixmap(name, QSize(width, height));
        return property.write(style, qVariantFromValue(pixmap));

    }  else if (containerType == CONST_SCALABLE_TYPE_0) {
        property.write(style, qVariantFromValue((const DuiScalableImage *) NULL));

    }  else if (containerType == CONST_SCALABLE_TYPE_1) {
        QString name;
        ds >> name;
        const DuiScalableImage *image = DuiTheme::scalableImage(name, 0, 0, 0, 0);
        return property.write(style, qVariantFromValue(image));
        ...
        const DuiScalableImage *image = DuiTheme::scalableImage(name, left, right, top, bottom);
        return property.write(style, qVariantFromValue(image));
        ...
        ds >> feedbackName;
        DuiFeedback feedback(feedbackName);
        return property.write(style, qVariantFromValue(feedback));
        ...
        int width = DuiStyleSheetAttribute::attributeToInt(widthStr, &conversionOK, WidthAttribute, orientation);
        int height = DuiStyleSheetAttribute::attributeToInt(heightStr, &conversionOK, HeightAttribute, orientation);
        return property.write(style, QSize(width, height));
        ...
        qreal width = DuiStyleSheetAttribute::attributeToFloat(widthStr, &conversionOK, WidthAttribute, orientation);
        qreal height = DuiStyleSheetAttribute::attributeToFloat(heightStr, &conversionOK, HeightAttribute, orientation);
...
void DuiStyleSheetAttribute::writeAttribute(const QMetaProperty &property,
        DuiSharedData &shm,
        QString filename)
{
    bool conversionOK = false;
    ...
    if (attributeType == types[BOOL_TYPE]) {
        ...
        int integer = DuiStyleSheetAttribute::attributeToInt(value, &conversionOK);
        if (conversionOK)
        ...
        qreal real = DuiStyleSheetAttribute::attributeToFloat(value, &conversionOK);
        ...
        if (value.startsWith(values[IMAGE_VALUE], Qt::CaseInsensitive)) {
            ...
            if (list.size() == 3) {
                ...
                qWarning("%s: %s; => %s: \"%s\" %s %s; //<-- %s:%d", name.toLatin1().trimmed().data(), value.toLatin1().data(), name.toLatin1().trimmed().data(), list[0].toLatin1().trimmed().data(), list[1].toLatin1().trimmed().data(), list[2].toLatin1().trimmed().data(), filename.toLatin1().data(), DuiStyleSheetParser::getLineNum(filename, position));

                // Image with size parameters
                QString name = list[0];
                int width = DuiStyleSheetAttribute::attributeToInt(list[1], &conversionOK);
                int height = DuiStyleSheetAttribute::attributeToInt(list[2], &conversionOK);
                ...
                //since 0.11
                qWarning("DEPRECATED: using \"image\" prefix for pixmap attributes is deprecated. Use following syntax instead:");
                qWarning("%s: %s; => %s: \"%s\"; //<-- %s:%d", name.toLatin1().trimmed().data(), value.toLatin1().data(), name.toLatin1().trimmed().data(), list[0].toLatin1().trimmed().data(), filename.toLatin1().data(), DuiStyleSheetParser::getLineNum(filename, position));
        ...
        if (value.startsWith(values[SCALABLE_VALUE], Qt::CaseInsensitive)) {
            ...
            if (list.size() == 5) {
                QString name = list[0];
                int left = DuiStyleSheetAttribute::attributeToInt(list[1], &conversionOK);
                int right = DuiStyleSheetAttribute::attributeToInt(list[2], &conversionOK);
                int top = DuiStyleSheetAttribute::attributeToInt(list[3], &conversionOK);
                int bottom = DuiStyleSheetAttribute::attributeToInt(list[4], &conversionOK);

                //since 0.11
                qWarning("DEPRECATED: using \"scalable\" prefix for images is deprecated. Use following syntax instead:");
                qWarning("%s: %s; => %s: \"%s\" %s %s %s %s; //<-- %s:%d", name.toLatin1().trimmed().data(), value.toLatin1().data(), name.toLatin1().trimmed().data(), name.toLatin1().trimmed().data(), list[1].toLatin1().trimmed().data(), list[2].toLatin1().trimmed().data(), list[3].toLatin1().trimmed().data(), list[4].toLatin1().trimmed().data(), filename.toLatin1().data(), DuiStyleSheetParser::getLineNum(filename, position));
        ...
        if (value.startsWith(values[SIZE_VALUE], Qt::CaseInsensitive)) {
            ...
            if (list.size() == 2) {
                ...
                qWarning("%s: %s; => %s: %s %s; //<-- %s:%d", name.toLatin1().trimmed().data(), value.toLatin1().data(), name.toLatin1().trimmed().data(), list[0].toLatin1().trimmed().data(), list[1].toLatin1().trimmed().data(), filename.toLatin1().data(), DuiStyleSheetParser::getLineNum(filename, position));
        ...
        if (value.startsWith(values[POS_VALUE], Qt::CaseInsensitive)) {
            ...
            if (list.size() == 2) {
                ...
                qWarning("%s: %s; => %s: %s %s; //<-- %s:%d", name.toLatin1().trimmed().data(), value.toLatin1().data(), name.toLatin1().trimmed().data(), list[0].toLatin1().trimmed().data(), list[1].toLatin1().trimmed().data(), filename.toLatin1().data(), DuiStyleSheetParser::getLineNum(filename, position));
                if (attributeType == types[POINT_TYPE]) {
                    int x = DuiStyleSheetAttribute::attributeToInt(list[0], &conversionOK);
                    int y = DuiStyleSheetAttribute::attributeToInt(list[1], &conversionOK);

                    shm << (int)POINT_TYPE << qVariantFromValue(QPoint(x, y));
                } else {
                    qreal x = DuiStyleSheetAttribute::attributeToFloat(list[0], &conversionOK);
                    qreal y = DuiStyleSheetAttribute::attributeToFloat(list[1], &conversionOK);
            ...
            if (list.size() == 2) {
                if (attributeType == types[POINT_TYPE]) {
                    int width = DuiStyleSheetAttribute::attributeToInt(list[0], &conversionOK);
                    int height = DuiStyleSheetAttribute::attributeToInt(list[1], &conversionOK);

                    shm << (int)POINT_TYPE << qVariantFromValue(QPoint(width, height));
                } else {
                    qreal width = DuiStyleSheetAttribute::attributeToFloat(list[0], &conversionOK);
                    qreal height = DuiStyleSheetAttribute::attributeToFloat(list[1], &conversionOK);
        ...
        if (value.startsWith(values[FONT_VALUE], Qt::CaseInsensitive)) {
            ...
            QStringList list = values.split(',');
            if (list.size() == 2) {
                float pixelSize = DuiStyleSheetAttribute::attributeToFloat(list[1], &conversionOK);
                if (conversionOK) {
                    QFont font(list[0]);
                    font.setPixelSize(pixelSize);
                    //since 0.11
            ...
            //family + weight + font size
                ...
                float pixelSize = DuiStyleSheetAttribute::attributeToFloat(list[2], &conversionOK);
                if (conversionOK) {
                    QFont font(list[0]);
                    font.setPixelSize(pixelSize);
                    if (DataTypeConverter.WEIGHTS.contains(list[1]))
