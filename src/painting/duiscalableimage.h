...
#ifndef DUISCALABLEIMAGE_H
#define DUISCALABLEIMAGE_H
...
/*!
  ...
  \brief DuiScalableImage is a class for drawing scaled pixmaps into screen
...
*/
class DUI_EXPORT DuiScalableImage : public QObject
{
    Q_OBJECT
    ...
    */
    ...
    /*!
      ...
      \param pixmapId - optional pixmapId associated with a scalable image which will be included into warnings
        if DuiScalableImage happen to find those
    */
    DuiScalableImage(const QPixmap *pixmap, int left, int right, int top, int bottom, const QString &pixmapId = "");

    /*!
      \brief Destructor.
     */
    virtual ~DuiScalableImage();
...
private:
    Q_DECLARE_PRIVATE(DuiScalableImage)
    Q_DISABLE_COPY(DuiScalableImage)
};
