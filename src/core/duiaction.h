...
/**
   \class DuiAction
   \brief DuiAction implements an extension of the QAction for libdui
 */
class DUI_EXPORT DuiAction : public QAction
{
    Q_OBJECT
...
public:
     ...
     */
    explicit DuiAction(QObject *parent);
     ...
     */
    DuiAction(const QString &text, QObject *parent);
    ...
    /**
       ...
       This overloaded constructor is provided for convenience.
     */
    DuiAction(const QString &iconID, const QString &text, QObject *parent);
...
protected:
    /** \internal */
    DuiActionPrivate *const d_ptr;
    DuiAction(DuiActionPrivate &dd, QObject *parent = 0);
    /** \internal_end */

private:
    Q_DISABLE_COPY(DuiAction)
    Q_DECLARE_PRIVATE(DuiAction)

#ifdef UNIT_TEST
    friend class Ut_DuiAction;
#endif
};
