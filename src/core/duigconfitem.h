...
#ifndef DUIGCONFITEM_H
#define DUIGCONFITEM_H
...
/*!
  ...
  \brief DuiGConfItem is a simple C++ wrapper for GConf.

  Creating a DuiGConfItem instance gives you access to a single GConf
  ...
  pass in a QVariant when setting the value.  DuiGConfItem converts
  ...
  \warning DuiGConfItem is as thread-safe as GConf.

*/

class DUI_EXPORT DuiGConfItem : public QObject
{
    Q_OBJECT

public:
    /*! Initializes a DuiGConfItem to access the GConf key denoted by
    ...
    */
    explicit DuiGConfItem(const QString &key, QObject *parent = 0);

    /*! Finalizes a DuiGConfItem.
     */
    virtual ~DuiGConfItem();
    ...
    /*! Set the value of this item to \a val.  If \a val can not be
        ...
        changedValue() signal is emitted on this DuiGConfItem as part
        of calling set(), but other DuiGConfItem:s for the same key do
...
private:
    friend struct DuiGConfItemPrivate;
    struct DuiGConfItemPrivate *priv;

    void update_value(bool emit_signal);
};

#endif // DUIGCONFITEM_H
