    ...
    valid(true)
{
    ...
    if (file.exists() && file.open(QIODevice::ReadOnly)) {
        ...
        duiWarning("DuiDesktopEntryPrivate") << "Specified Desktop file does not exist" << fileName;
    }
}

DuiDesktopEntryPrivate::~DuiDesktopEntryPrivate()
{
}

bool DuiDesktopEntryPrivate::readDesktopFile(QIODevice &device, QMap<QString, QString> &desktopEntriesMap)
{
            ...
            if (keyValueRE.exactMatch(line) && !currentGroup.isEmpty()) {
                ...
                if (!desktopEntriesMap.contains(desktopKey)) {
                    ...
                    if (desktopKey == CategoriesKey || desktopKey == OnlyShowInKey ||
                        ...
                        if (value.endsWith("\\;") || !value.endsWith(';')) {
                            ...
                            duiWarning("DuiDesktopEntryPrivate") << "Value for multivalue key" << desktopKey << "does not end in a semicolon";
                            valid = false;
                        }
                    }
                    ...
                    duiWarning("DuiDesktopEntryPrivate") << "Key" << desktopKey << "already defined. Value" << keyValueRE.cap(2) << "is ignored";
                }
                ...
                if (!groupNames.contains(groupHeaderRE.cap(1), Qt::CaseSensitive)) {
                    if (groupNames.isEmpty() && groupHeaderRE.cap(1) != "Desktop Entry") {
                        duiWarning("DuiDesktopEntryPrivate") << "Desktop entry should start with group name \"Desktop Entry\" ";
                        valid = false;
                    } else {
                ...
                // Redefining a group name will cause the desktop entry to become invalid but still parsed by the parser.
                    ...
                    currentGroup = groupHeaderRE.cap(1);
                    duiWarning("DuiDesktopEntryPrivate") << "Multiple definitions of group" << groupHeaderRE.cap(1);
                    valid = false;
                }
            } else {
                duiWarning("DuiDesktopEntryPrivate") << "Invalid .desktop entry line:" << line;
            }
        }
    }
    return valid;
...
bool DuiDesktopEntryPrivate::boolValue(const QString &key) const
{
    return desktopEntriesMap.value(key) == "true";
}

QStringList DuiDesktopEntryPrivate::stringListValue(const QString &key) const
{
    QStringList list;
...
DuiDesktopEntry::DuiDesktopEntry(const QString &fileName) :
    d_ptr(new DuiDesktopEntryPrivate(fileName))
{
}

DuiDesktopEntry::DuiDesktopEntry(DuiDesktopEntryPrivate &dd) :
    d_ptr(&dd)
{
}
