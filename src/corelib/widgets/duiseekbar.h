...
#include "duislider.h"
#include <duiseekbarmodel.h>

/*!
    \class DuiSlider
    \brief DuiSeekBar is a special type of DuiSlider used for displaying playback status for multimedia content.

    \ingroup widgets

    \section DuiSeekBar Overview
        Seekbar includes received and non-received parts to visualize e.g. streamed/buffered content.

\section DuiSeekBarExampleCodes Example codes
    ...
    \sa DuiSeekBarModel DuiSliderStyle
*/

class DUI_EXPORT DuiSeekBar : public DuiSlider
{
    Q_OBJECT
    DUI_CONTROLLER(DuiSeekBar)

    /*!
        \property DuiSlider::loadedContentMinimum
        \brief Minimum value in seekbar loaded content range.

        See DuiSeekBarModel::loadedContentMin for details.
    */
    Q_PROPERTY(int loadedContentMinimum READ loadedContentMinimum WRITE setLoadedContentMinimum)

    /*!
        \property DuiSlider::loadedContentMaximum
        \brief Maximum value in seekbar loaded content range.

        See DuiSeekBarModel::loadedContentMax for details.
    */
...
public:
     ...
     */
    explicit DuiSeekBar(QGraphicsItem *parent = 0, DuiSeekBarModel *model = 0);
