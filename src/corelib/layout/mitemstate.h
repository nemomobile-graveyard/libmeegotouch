...
public:
     ...
     *
     * Returns a Null MItemState item.
     */
    MItemState();
    /*!
     * \brief Constructs an item state.
    ...
    /* Copy constructor */
    MItemState(const MItemState &);
    /* Assignment operator */
    MItemState *operator =(const MItemState &);
     ...
     * information stored in the MItemState.
     *
     * @param other The item to compare with this one.
     *
     * @return true if both MItemStates refer to the same object and
     *         false otherwise.
     */
    bool operator ==(const MItemState &other) const;
    ...
    /*!
     * \brief Checks whether this is a null MItemState item?
     *
     * @return true if this MItemState item has a null QGraphicsWidget
     *              and false otherwise.
     */
    bool isNull() const;
     ...
     */
    bool hasIdenticalLayout(const MItemState &) const;
...
protected:
    MItemStatePrivate *const d_ptr;

private:
    Q_DECLARE_PRIVATE(MItemState)
};


Q_DECLARE_OPERATORS_FOR_FLAGS(MItemState::Flags)
