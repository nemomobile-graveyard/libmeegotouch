...
#include "duiassembly_p.h"
#include "duidebug.h"
#include "duiwidgetcontroller.h"

bool DuiAssemblyPrivate::loadViewConfiguration(const QStringList &themeInheritance)
{
    ...
    foreach(const QString & themePath, themeInheritance) {
        ...
        QString filename = themePath + "dui" + QDir::separator() + name + QDir::separator() + name + ".conf";
        ...
        if (config.status() != QSettings::NoError) {
            duiWarning("DuiAssembly") << "Invalid view configuration file:" << filename;
            return false;
        }
...
void DuiAssemblyPrivate::loadStylesheet(const QString &filename, const DuiLogicalValues &logicalValues)
{
    ...
    if (result) {
        ...
        duiWarning("DuiAssembly") << "Failed to load stylesheet:" << filename;
        delete newStylesheet;
        newStylesheet = NULL;
    }
}

void DuiAssemblyPrivate::loadStylesheets(const QStringList &themeInheritance, const DuiLogicalValues &logicalValues)
{
    delete stylesheet;
    stylesheet = NULL;
    ...
    for (int j = themeInheritance.count() - 1; j >= 0; --j) {
        ...
        QString path = themeInheritance.at(j) + "dui" + QDir::separator() + name + QDir::separator();
        ...
        if (QFile::exists(filename)) {
            ...
            if (j == themeInheritance.count() - 1) {
                ...
                duiWarning("DuiAssembly") << "Stylesheet missing!" << filename;
            }
        }
    }
}

DuiAssembly::DuiAssembly(const QString &name) :
    d_ptr(new DuiAssemblyPrivate)
{
    d_ptr->name = name;
    d_ptr->stylesheet = NULL;
}

DuiAssembly::~DuiAssembly()
{
    delete d_ptr->stylesheet;
    delete d_ptr;
}

QString DuiAssembly::name() const
{
    return d_ptr->name;
}

QString DuiAssembly::viewType(const DuiWidgetController *widget, bool &exactMatch) const
{
...
DuiStyleSheet *DuiAssembly::stylesheet() const
{
    return d_ptr->stylesheet;
}

void DuiAssembly::themeChanged(const QStringList &themeInheritance, const DuiLogicalValues &logicalValues)
{
